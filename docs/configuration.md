...
| `detector.background_photon_rate_per_s` | Non-negative additive incident background in photons/s/pixel, uniform over the detector (default `0.0`). Passed straight to `getframes` as its `background` term, so it collects charge and carries shot noise like any other light. Use it for sky and thermal background: `makewfs` does not know a field stop's solid angle or a sky surface brightness, so the caller computes the rate. It is *incident* light, not detector dark current, which the camera preset already owns. |
